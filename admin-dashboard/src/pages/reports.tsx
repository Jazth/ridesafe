...
const confirmAdminAction = async () => {
  ...
  try {
    ...
    if (action === "flag") {
      updateData.flags = (mechanicSnap.data().flags || 0) + 1;
      actionText = "flagged";
    }
  ...
  `,
    ...
    setFeedback({ type: "success", message: `Mechanic successfully ${actionText}.` }); // ✅ Show success
  } catch (err) {
  ...
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(q, (snapshot) => {
      const data = snapshot.docs.map((docSnap) => {
        ...
        const mappedStatus =
          rawStatus.toLowerCase() === "done" ? "completed" : "saved";

        const report: Report = {
          id: docSnap.id,
          userId: d.userId || d.breakdownRequest?.userId,
          userName: d.reportedBy?.name || "Anonymous",
          mechanicId:
            d.mechanicId ||
            d.breakdownRequest?.mechanicId ||
            d.reportedTo?.id ||
            "",
  ...
  >
    ...
    >
      ✕
        ...
        <h1 className="text-3xl font-bold text-[#FF5722]">Breakdown Reports</h1>
        ...
        <div className="space-y-3">
          {filteredReports.map((report) => (
            ...
            >
              ...
              <p>
                <span className="font-semibold">Issue:</span> {report.issue}
              </p>
        ...
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50">
          <div className="bg-white p-6 rounded-2xl shadow-xl w-[90%] max-w-md relative overflow-y-auto max-h-[90vh]">
            ...
            <div className="space-y-2 text-gray-700">
              ...
              <p>
                <span className="font-semibold">Reported Mechanic:</span>{" "}
