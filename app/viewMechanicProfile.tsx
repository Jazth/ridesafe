...
export default function ViewMechanicProfile() {
  ...
  const [selectedReasons, setSelectedReasons] = useState<string[]>([]); // ✅ multi-select
  ...
  return (
    <SafeAreaView style={{ flex: 1, backgroundColor: "white", paddingTop: 50 }}>
      ...
      >
        ...
        >
          <Text style={{ fontSize: 24, fontWeight: "bold", marginBottom: 12, color: "#FF5722" }}>
            Mechanic Details
          </Text>
          {request.claimedBy ? (
            <>
              <Text style={styles.detailText}>👨‍🔧 Name: {request.claimedBy.name || "N/A"}</Text>
              <Text style={styles.detailText}>📞 Phone: {request.claimedBy.phoneNum || "N/A"}</Text>
              <Text style={styles.detailText}>🏢 Business: {request.claimedBy.business || "N/A"}</Text>
              <Text style={styles.detailText}>📍 Address: {request.claimedBy.serviceArea || "N/A"}</Text>
            </>
          ) : (
        ...
        >
          ...
          {request.timestamp?.seconds && (
            <Text style={styles.detailText}>
              📅 Date: {new Date(request.timestamp.seconds * 1000).toLocaleString()}
            </Text>
          )}
        </View>
      ...
      <Modal visible={reportVisible} transparent animationType="slide" onRequestClose={() => setReportVisible(false)}>
        <View style={{ flex: 1, justifyContent: "flex-end", backgroundColor: "rgba(0,0,0,0.5)" }}>
          <View style={{ backgroundColor: "white", borderTopLeftRadius: 20, borderTopRightRadius: 20, padding: 25, minHeight: 550 }}>
            <Text style={{ fontSize: 22, fontWeight: "bold", color: "#FF5722", marginBottom: 15 }}>
            ...
            <TouchableOpacity
              onPress={pickImage}
              style={{ backgroundColor: "#FFE0CC", padding: 12, borderRadius: 8, alignItems: "center", marginTop: 10 }}
            >
            ...
            <Pressable
              ...
              style={{ backgroundColor: "#FF5722", marginTop: 25, borderRadius: 10, paddingVertical: 12, alignItems: "center" }}
            >
              <Text style={{ color: "white", fontWeight: "bold", fontSize: 16 }}>
                Submit Report
              </Text>
            ...
            >
              <Text style={{ color: "#FF5722", fontWeight: "bold", fontSize: 15 }}>
                Cancel
              </Text>
            </Pressable>
          </View>
